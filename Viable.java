...
public class ViableProduct {
 public static class ViableProductMapper extends Mapper<LongWritable,Text,Text,Text>
 {
	...
	context.write(new Text(record[5]), new Text(finalvalue));
	}
 }
 public static class ViableProductPartitioner extends
 ...
 public static class ViableProductReducer extends Reducer<Text,Text,NullWritable,Text>
 {
	 
	 private TreeMap<Double, Text> ViableProductMap = new TreeMap<Double, Text>();
	 ...
	 protected void reduce(Text key,Iterable <Text> value,Context context) throws IOException, InterruptedException
		{
		 ...
		 ViableProductMap.put(margin, new Text(answer));
		 
		 if(ViableProductMap.size()>5)ViableProductMap.remove(ViableProductMap.firstKey()); 
		 
					 
		}	 
	 protected  void cleanup (Context context) throws IOException, InterruptedException  
		{
			for (Text t : ViableProductMap.descendingMap().values()) 
			{
 ...
 job.setJarByClass(ViableProduct.class);
 job.setMapperClass(ViableProductMapper.class);
 job.setPartitionerClass(ViableProductPartitioner.class);
 job.setReducerClass(ViableProductReducer.class);
